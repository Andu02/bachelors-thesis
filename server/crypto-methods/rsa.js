...
export function encrypt(text, params = { p: 61, q: 53, e: 17 }) {
  ...
  const d = modInverse(e, phi); // Nu este folosit aici, dar poate fi folosit la decriptare

  // Criptare caracter cu caracter
  return text
    .split("")
    .map((char) => {
      const m = BigInt(char.charCodeAt(0));
      const c = m ** e % n;
      return c.toString(); // fiecare caracter criptat e salvat ca string
    })
    .join(","); // separăm caracterele criptate cu virgulă
}
